...
public class CDMIClient {
	...
	public static void main(String[] args) throws UnrecoverableKeyException, CertificateException, NoSuchAlgorithmException, KeyStoreException, URISyntaxException, KeyManagementException {
		...
		try {
			...
			response = pdcStorageConn.getContainerProxy().create(level1,
					parameters);
			responseCode = response.getStatusLine().getStatusCode();

			response = pdcStorageConn.getContainerProxy().create(level2,
					parameters);
			responseCode = response.getStatusLine().getStatusCode();
			...
			response = pdcStorageConn.getBlobProxy().create(
					level2 + data1.getName(), Utils.getBytesFromFile(data1),
					parameters);
			responseCode = response.getStatusLine().getStatusCode();

			response = pdcStorageConn.getBlobProxy().create(
					level2 + data2.getName(), Utils.getBytesFromFile(data2),
					parameters);
			responseCode = response.getStatusLine().getStatusCode();
			...
			response = pdcStorageConn.getNonCdmiContainerProxy().create(level1,
					parameters);
			responseCode = response.getStatusLine().getStatusCode();

			pdcStorageConn.getContainerProxy().create(level2, parameters);
			...
			for (String s : pdcStorageConn.getContainerProxy().getChildren(
					level1)) {
	...
	static void prepareData(CDMIConnection localFileBackendConn,
		...
		response = remoteAWSBackendConn.getContainerProxy().create(cdmiBase,
				parameters);
		responseCode = response.getStatusLine().getStatusCode();
		System.out.println(cdmiBase + " created: " + responseCode);

		response = remoteAWSBackendConn.getNonCdmiContainerProxy().create(
				nonCdmiBase, parameters);
		responseCode = response.getStatusLine().getStatusCode();
		System.out.println(nonCdmiBase + " created: " + responseCode);
		...
		response = localFileBackendConn.getBlobProxy().create(blobPath, value,
				parameters);
		responseCode = response.getStatusLine().getStatusCode();
		...
		response = localFileBackendConn.getNonCdmiBlobProxy().create(blobPath,
				value, parameters);
		responseCode = response.getStatusLine().getStatusCode();
		System.out.println(blobPath + " created: " + responseCode);
